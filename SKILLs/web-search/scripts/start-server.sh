...
resolve_node_runtime() {
  ...
  if [ -n "${DIOSCLAW_ELECTRON_PATH:-}" ] && [ -x "${DIOSCLAW_ELECTRON_PATH}" ]; then
    NODE_CMD="$DIOSCLAW_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from DiosClaw."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from DiosClaw so scripts can use Electron runtime."
  exit 1
fi
