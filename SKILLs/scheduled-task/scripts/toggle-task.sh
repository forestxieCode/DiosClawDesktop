#!/bin/bash
# Enable or disable a scheduled task via DiosClaw internal API.
...
#
# Environment variables (set automatically by DiosClaw cowork session):
#   DIOSCLAW_API_BASE_URL - Internal proxy URL (always points to local proxy)
...
resolve_http_node_runtime() {
  ...
  if [ -n "${DIOSCLAW_ELECTRON_PATH:-}" ] && [ -x "${DIOSCLAW_ELECTRON_PATH}" ]; then
    HTTP_NODE_CMD="$DIOSCLAW_ELECTRON_PATH"
    HTTP_NODE_ARGS=()
...
if [ -z "$DIOSCLAW_API_BASE_URL" ]; then
  echo '{"success":false,"error":"DIOSCLAW_API_BASE_URL not set. This script must run inside a DiosClaw cowork session."}'
  exit 1
fi
...
# DIOSCLAW_API_BASE_URL always points to the local proxy: http://127.0.0.1:PORT
BASE_URL="${DIOSCLAW_API_BASE_URL%/}"
