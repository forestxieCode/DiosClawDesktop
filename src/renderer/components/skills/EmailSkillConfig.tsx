...
const EmailSkillConfig: React.FC<EmailSkillConfigProps> = ({ onClose }) => {
  ...
  const inputClassName = 'block w-full rounded-xl bg-surface-inset dark:bg-dark-surface-inset dark:border-dark-border border-border border focus:border-primary focus:ring-1 focus:ring-primary/30 dark:text-dark-text text-text-primary px-3 py-2 text-xs';
  const labelClassName = 'block text-xs font-medium dark:text-dark-text text-text-primary mb-1';

  if (loading) {
    return (
      <div className="p-4 text-xs dark:text-dark-text-secondary text-text-secondary">
  ...
  return (
    <div className="space-y-4 p-4 rounded-xl border dark:border-dark-border border-border dark:bg-dark-surface/30 bg-surface/30">
      <div className="flex items-center justify-between">
        ...
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="text-xs dark:text-dark-text-secondary text-text-secondary hover:text-primary transition-colors"
          >
      ...
      <div className="min-h-[18px]">
        {(persistError || (isPersisting && showPersisting)) && (
          <div className={`text-xs ${persistError ? 'text-red-600 dark:text-red-400' : 'text-text-secondary dark:text-dark-text-secondary'}`}>
      ...
      {/* Hint */}
      {hintKey && (
        <div className="text-xs dark:text-dark-text-secondary text-text-secondary bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg px-3 py-2">
      ...
      <button
        ...
        className="flex items-center gap-1 text-xs dark:text-dark-text-secondary text-text-secondary hover:text-primary transition-colors"
      >
        {showAdvanced ? (
      ...
      {/* Advanced Settings */}
      {showAdvanced && (
        <div className="space-y-3 pl-2 border-l-2 border-border dark:border-dark-border">
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 text-xs dark:text-dark-text text-text-primary">
              <input
                type="checkbox"
