...
const PathTooltip: React.FC<PathTooltipProps> = ({ path, anchorRect, visible }) => {
  ...
  return (
    <div
      style={style}
      className="px-3.5 py-2.5 text-[13px] leading-relaxed rounded-xl shadow-xl dark:bg-dark-bg bg-page dark:text-dark-text text-text-primary dark:border-dark-border border-border border break-all pointer-events-none"
    >
      {path}
    </div>
  );
...
const FolderSelectorPopover: React.FC<FolderSelectorPopoverProps> = ({
  ...
  return (
    <>
      ...
      <div
        ref={popoverRef}
        className="absolute bottom-full left-0 mb-2 w-56 rounded-lg border dark:border-dark-border border-border dark:bg-dark-surface bg-surface shadow-lg z-50"
      >
        {/* Add Folder option */}
        <button
          onClick={handleAddFolder}
          className="w-full flex items-center gap-3 px-3 py-2.5 text-sm dark:text-dark-text text-text-primary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors rounded-t-lg"
        >
          <FolderPlusIcon className="h-4 w-4 dark:text-dark-text-secondary text-text-secondary" />
        ...
        >
          <button
            className="w-full flex items-center justify-between gap-3 px-3 py-2.5 text-sm dark:text-dark-text text-text-primary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors rounded-b-lg"
          >
            <div className="flex items-center gap-3">
              <ClockIcon className="h-4 w-4 dark:text-dark-text-secondary text-text-secondary" />
              <span>{i18nService.t('recentFolders')}</span>
            </div>
            <ChevronRightIcon className="h-3 w-3 dark:text-dark-text-secondary text-text-secondary" />
          </button>
        </div>
      </div>
      ...
      {showRecentSubmenu && (
        <div
          ref={submenuRef}
          className="fixed w-64 max-h-80 overflow-y-auto rounded-lg border dark:border-dark-border border-border dark:bg-dark-surface bg-surface shadow-lg z-[60]"
          style={{ top: submenuPosition.top, left: submenuPosition.left }}
          onMouseEnter={() => setShowRecentSubmenu(true)}
          onMouseLeave={() => setShowRecentSubmenu(false)}
        >
          {isLoading ? (
            <div className="px-3 py-2.5 text-sm dark:text-dark-text-secondary text-text-secondary">
            ...
            recentFolders.map((folder, index) => (
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-sm dark:text-dark-text text-text-primary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors text-left first:rounded-t-lg last:rounded-b-lg"
              >
                <FolderIcon className="h-4 w-4 flex-shrink-0 dark:text-dark-text-secondary text-text-secondary" />
