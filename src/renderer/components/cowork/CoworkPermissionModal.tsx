...
const CoworkPermissionModal: React.FC<CoworkPermissionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center modal-backdrop">
      <div className="modal-content w-full max-w-lg mx-4 dark:bg-dark-surface bg-surface rounded-2xl shadow-modal overflow-hidden">
        {/* Header */}
        ...
        <div className="px-6 py-4 space-y-4 max-h-[60vh] overflow-y-auto">
          {isQuestionTool ? (
              ...
              {questions.map((question) => {
                ...
                return (
                  <div
                    ...
                    className="rounded-xl border dark:border-dark-border border-border p-4 space-y-3"
                  >
                    <div className="flex items-start gap-2">
                      {question.header && (
                        <span className="text-[11px] uppercase tracking-wide px-2 py-1 rounded-full bg-surface-hover dark:bg-dark-surface-hover dark:text-dark-text-secondary text-text-secondary">
                          {question.header}
                        </span>
                      )}
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left rounded-lg border px-3 py-2 transition-colors ${
                              isSelected
                                ? 'border-primary bg-primary/10 text-text-primary dark:text-dark-text'
                                : 'border-border dark:border-dark-border dark:text-dark-text-secondary text-text-secondary hover:bg-surface-hover dark:hover:bg-dark-surface-hover'
                            }`}
                          >
            ...
            <>
              {/* Tool name */}
              <div>
                <label className="block text-xs font-medium dark:text-dark-text-secondary text-text-secondary uppercase tracking-wider mb-1">
                  {i18nService.t('coworkToolName')}
                </label>
                <div className="px-3 py-2 rounded-lg dark:bg-dark-bg bg-page">
                  <code className="text-sm dark:text-dark-text text-text-primary">
                    {permission.toolName}
                  </code>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="px-3 py-2 rounded-lg dark:bg-dark-bg bg-page max-h-48 overflow-y-auto">
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t dark:border-dark-border border-border">
          <button
            onClick={handleDeny}
          ...
          <button
            ...
            disabled={!isComplete}
            className="px-4 py-2 text-sm font-medium rounded-lg bg-primary hover:bg-primary-light text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {approveButtonLabel}
          </button>
        </div>
