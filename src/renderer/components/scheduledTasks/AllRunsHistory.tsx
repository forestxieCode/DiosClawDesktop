...
const AllRunsHistory: React.FC = () => {
  ...
  if (allRuns.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-6">
        ...
        <p className="text-sm font-medium dark:text-dark-text-secondary text-text-secondary">
  ...
  return (
    <div>
      {/* Column Headers */}
      <div className="grid grid-cols-[1fr_1fr_80px] items-center gap-3 px-4 py-2 border-b dark:border-dark-border/50 border-border/50">
      ...
      {allRuns.map((run) => {
        ...
        return (
          <div
            key={run.id}
            className={`grid grid-cols-[1fr_1fr_80px] items-center gap-3 px-4 py-3 border-b dark:border-dark-border/50 border-border/50 transition-colors ${
              run.sessionId
                ? 'hover:bg-surface-hover/50 dark:hover:bg-dark-surface-hover/50 cursor-pointer'
                : ''
            }`}
            onClick={() => handleViewSession(run)}
          >
            {/* Task title */}
            <div className="text-sm dark:text-dark-text text-text-primary truncate">
              {run.taskName}
              {run.status === 'running' && (
      ...
      {allRuns.length >= 50 && allRuns.length % 50 === 0 && (
        <button
          ...
          className="w-full py-3 text-sm text-primary hover:text-primary-light transition-colors"
        >
